...
training_input_raw <- read_excel("ResearchPapers/Richards_Joey2/Richards_Joey/trainInputs2.xlsx",  col_names = FALSE, col_types = "numeric")
training_output_raw <- read_excel("ResearchPapers/Richards_Joey2/Richards_Joey/trainOutputs250.xlsx", col_names = FALSE, col_types = "numeric")
testing_input_raw <- read_excel("ResearchPapers/Richards_Joey2/Richards_Joey/testInputs2.xlsx",  col_names = FALSE, col_types = "numeric")
testing_output_raw <- read_excel("ResearchPapers/Richards_Joey2/Richards_Joey/testOutputs250.xlsx",  col_names = FALSE, col_types = "numeric")

pts <- seq(0.003,0.3, by = 0.003)

training_input <- cbind(training_input_raw$...1, training_input_raw$...2, training_input_raw$...3, training_input_raw$...4)

numCurves <- 10
trainInputs <- training_input[1:numCurves,]
trainOutputs <- training_output_raw[1:numCurves,] #first n input sets

###NORMALIZE INPUTS###
trainInputsRaw = trainInputs
#col 1 is from 0.3 to 0.5
trainInputs[,1] = (trainInputs[,1] - 0.3)/(0.5 - 0.3)
#col 2 is from 1/1800 to 1/36
trainInputs[,2] = (trainInputs[,2] - (1/1800))/((1/36) - (1/1800))
#col 3 is from -900 to -150
trainInputs[,3] = (trainInputs[,3] - (-900))/(-150 - (-900))
#col 4 is from 0.32 to 5
trainInputs[,4] = (trainInputs[,4] - 0.32)/(5 - 0.32)

#removing the third column whose range parameter is massive
trainInputs <- cbind(trainInputs[,1:2], trainInputs[,4])

xVals <- pts #really y vals in terms of the column
yVals <- trainOutputs
...
testIndex = 200
temp <- as.double(testing_input_raw[testIndex,])
#col 1 is from 0.3 to 0.5
temp[1] = (temp[1] - 0.3)/(0.5 - 0.3)
#col 2 is from 1/1800 to 1/36
temp[2] = (temp[2] - (1/1800))/((1/36) - (1/1800))
#col 3 is from -900 to -150
temp[3] = (temp[3] - (-900))/(-150 - (-900))
#col 4 is from 0.32 to 5
temp[4] = (temp[4] - 0.32)/(5 - 0.32)
#removing the third column
temp <- cbind(temp[1], temp[2], temp[4])

testInputs <- temp
numTest <- dim(testInputs)[1]

testTrendMat <- cbind(temp, 1)
...
names(yVals) <- NULL
wT <- rbind(yVals[,20],yVals[,50], yVals[,80])
wT <- matrix(wT,ncol = 1)
zT <- rbind(pts[20], pts[50], pts[80])
for (i in 2:numCurves) {
  zT <- rbind(zT, pts[20], pts[50], pts[80])
}
yT <- wT
in_all <- cbind(wT, zT)
zT <- in_all[,2]
...
tmp <- data.frame(testing_output_raw[testIndex,])
names(tmp) <- NULL
tmp <- t(tmp)
test_all <- cbind(tmp,pts)
...
for (j in 1:N) {
  for (i in 1:N) {
    if (i == j) {
      ...
      # rho_f = cKSingle(testInputs[i,1],testInputs[j,1],fGamma[1])*cKSingle(testInputs[i,2],testInputs[j,2],fGamma[2])*cKSingle(testInputs[i,3],testInputs[j,3],fGamma[3])
      rho_f = cKSingle(testInputs[1],testInputs[1],fGamma[1])*cKSingle(testInputs[2],testInputs[2],fGamma[2])*cKSingle(testInputs[3],testInputs[3],fGamma[3])
      tempSig = sqrt(sigma2Ls[i] - 2*rho_f*sqrt(sigma2Ls[i])*sqrt(sigma2Ls[j]) + sigma2Ls[j])
      rho_eta = xiSingle(muLs[i], tempSig, gamma[1], muLs[j])
      rho_eta = cKSingle(test_all[i,2], test_all[j,2], gamma[2])
      # A[i,j] = A[i,j] * sqrt(sigma2Ls[i]) * sqrt(sigma2Ls[j]) * rho_eta * cKSingle(test_all[i,2], test_all[j,2], gamma[2])
      A[i,j] = A[i,j] * sqrt(sigma2Ls[i]) * sqrt(sigma2Ls[j]) * rho_eta
    }
  }
  for (i in 1:ndp) {
    for (k in 1:numVars) {
...
df2 <- data.frame(x = gTestInputs[,2], mu = muLs, lb = muLs - 1.96*sqrt(sigma2Ls), ub = muLs + 1.96*sqrt(sigma2Ls), trueVals = gTestInputs[,1], ppgaspPred = t(pred$mean), plb = t(pred$lower95), pub = t(pred$upper95))
...
# g2 <- g2 + geom_line(data = df2, aes(x = x, y = ppgaspPred), lwd=1, color='red')
# g2 <- g2 + geom_line(data = df2, aes(x = x, y = plb), lwd=1, linetype = "dashed", color='gray')
# g2 <- g2 + geom_line(data = df2, aes(x = x, y = pub), lwd=1, linetype = "dashed", color='gray')
g2 <- g2 + theme(legend.position = "none")
g2 <- g2 + xlab("Depth") + ylab(expression(theta))
g2 <- g2 + coord_flip()
